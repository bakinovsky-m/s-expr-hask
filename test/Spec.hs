...
main = hspec $ do
  ...
  describe "parse_and_eval" $ do
    ...
    it "parses op with sum" $
      parse_and_eval sexpr "(+ 1 2 3)" `shouldBe` Right (sum [1,2,3])
    it "parses op with extra spaces" $
      parse_and_eval sexpr "(+ 1 2     3 (- (2) 1))" `shouldBe` Right (sum [1,2,3,2-1])
